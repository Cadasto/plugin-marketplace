...
2. Add the entry, copying `description` / `version` / `keywords` from its `plugin.json` and pinning `source.ref` to its latest tag. Confirm the tag exists on the remote first; see [testing.md](testing.md#verifying-a-pin-before-committing).
